...
class PackersEditorDialog:
    """Диалог редактирования списка упаковщиков"""
    ...
    def show(self):
        ...
        self.window.title("Список упаковщиков")
        ...
        # Создаем canvas и scrollbar для списка упаковщиков
        ...
        for packer in current_packers:
            self._create_packer_row(scrollable_frame, packer)

        # Добавляем пустое поле для нового упаковщика
        self._create_packer_row(scrollable_frame, "")
        ...
        ttk.Button(
            button_frame,
            text="💾 Сохранить",
            command=self.save_packers_list
        ).pack(side=tk.LEFT, padx=5)

        ttk.Button(
            button_frame,
            text="➕ Добавить строку",
            command=lambda: self._create_packer_row(scrollable_frame, "")
        ).pack(side=tk.LEFT, padx=5)

        # Привязка Enter к сохранению
        self.window.bind("<Return>", lambda e: self.save_packers_list())

    def _create_packer_row(self, parent, packer):
        """Создает строку с полем ввода для упаковщика"""
        ...
        ttk.Button(
            ...
            text="×",
            width=2,
            command=lambda: self._remove_packer_row(row_frame, entry)
        ).pack(side=tk.RIGHT)

    def _remove_packer_row(self, row_frame, entry):
        """Удаляет строку с полем ввода"""
        if len(self.packer_entries) > 1:
            row_frame.destroy()
            self.packer_entries.remove(entry)
    ...
    def save_packers_list(self):
        """Сохраняет список упаковщиков"""
        try:
            ...
            # Обновляем список упаковщиков
            settings["packers"] = new_packers

            # Сохраняем обратно в файл
            if self.config_manager.save_json_settings("shared_utils.json", settings):
                if self.status_var:
                    self.status_var.set("✅ Список упаковщиков успешно обновлен!")
            ...
            if self.status_var:
                self.status_var.set(f"❌ Ошибка сохранения упаковщиков: {str(e)}")